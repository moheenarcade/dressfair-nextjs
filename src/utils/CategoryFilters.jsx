...
//         if (label === "Color" || label === "Size" || label === "Type") {
...
//                                         className=" cursor-pointer rounded text-[14px] flex flex-col items-center gap-2"
//                                     >
//                                         <div
//                                             className={`px-3 py-1 rounded-full border-[2px] p-[2px] transition-all duration-150 ${selectedFilters.Type.includes(type) ? "border-black scale-110" : "border-gray-300"
//                                                 } flex items-center justify-center`}
//                                         >
//                                             <span className="font-semibold text-center">{type}</span>
//                                         </div>
...
const CategoryFilters = () => {
    ...
    // Store the position of each filter button
    const [buttonPositions, setButtonPositions] = useState({});

    useOutsideClick(dropdownRef, () => setOpenDropdown(null));

    const handleSelect = (label, value) => {
        if (["Color", "Size", "Type", "Style", "Material", "Closure Type", 
             "Dress Length", "Neck Style", "Sleeve Length", "Pattern", 
             "Occasion", "Season"].includes(label)) {
            setSelectedFilters((prev) => {
                const alreadySelected = prev[label].includes(value);
                const newValues = alreadySelected
                    ? prev[label].filter((v) => v !== value)
    ...
    // Get button position for caret placement
    const getButtonPosition = (label) => {
        return buttonPositions[label] || 50; // default to 50% if not found
    };

    const renderDropdownContent = (label) => {
        const getOptions = () => {
            switch (label) {
                case "Sort by: Relevance": return sortOptions;
                ...
                case "Closure Type": return closureOptions;
                case "Dress Length": return dressLengthOptions;
                case "Neck Style": return neckStyleOptions;
                case "Sleeve Length": return sleeveLengthOptions;
                case "Pattern": return patternOptions;
                case "Occasion": return occasionOptions;
                ...
                <ul className={`flex flex-wrap gap-3 border-b border-gray-200 pb-4 ${
                    ...
                    {options.map((option, idx) => (
                        <li
                            ...
                            onClick={() => handleSelect(label, option)}
                            className={`cursor-pointer rounded text-[14px] flex flex-col items-center gap-2 ${
                                label === "Color" ? "px-3 py-2" : ""
                            }`}
                        >
                            {label === "Color" ? (
                                <>
                                    <div
                                        className={`w-14 h-14 rounded-full border-[2px] p-[2px] transition-all duration-150 ${
                                            selectedFilters[label].includes(option) 
                                ...
                                <div
                                    className={`px-4 py-2 rounded-full border-[2px] transition-all duration-150 ${
                                        selectedFilters[label].includes(option) 
    ...
    const isFullWidthDropdown = (label) => {
        return !["Sort by: Relevance", "Filters"].includes(label);
    };

    // Update button positions when dropdown opens
    const handleDropdownToggle = (label, event) => {
        if (openDropdown === label) {
            setOpenDropdown(null);
        } else {
            // Calculate button position for caret
            if (event && event.currentTarget) {
                ...
                if (scrollContainer) {
                    ...
                    setButtonPositions(prev => ({
                        ...prev,
                        [label]: Math.max(10, Math.min(90, percentage)) // Clamp between 10% and 90%
                    }));
                }
            }
        ...
        <div ref={dropdownRef} className="relative w-full">
            <div id="filter-scroll" className="flex items-center gap-2 px-2 relative w-full">
                ...
                {openDropdown && isFullWidthDropdown(openDropdown) && (
                    <div className="absolute left-0 right-0 top-[44px] z-[9999] w-full">
                        <div className="relative w-full mx-auto rounded-lg bg-white p-4 shadow-xl border border-gray-200">
                            {/* Dynamic Caret positioned above the clicked button */}
                            <div 
                                ...
                                style={{ 
                                    left: `${getButtonPosition(openDropdown)}%` 
                                }}
                            ></div>
